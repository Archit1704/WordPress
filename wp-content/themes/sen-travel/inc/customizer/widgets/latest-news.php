...
function sen_travel_popular_news() {
	register_widget( 'Sen_Travel_Latest_News' );
}
add_action( 'widgets_init', 'sen_travel_popular_news' );

class Sen_Travel_Latest_News extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_popular = array(
		  'classname'   => 'popular-news',
		  'description' => esc_html__( 'Add Widget to Display Popular Posts.', 'sen-travel' )
		);
		parent::__construct( 'Sen_Travel_Latest_News',esc_html__( 'St: Popular Posts', 'sen-travel' ), $widget_popular, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'Popular Posts', 'sen-travel' ),		
			  'category'       	=> '', 
			  'number'          => 5, 
			  'show_category'	=> true,	
			) 
		);
		$title     			= isset( $instance['title'] ) ? esc_html( $instance['title'] ) : esc_html__( 'Popular Posts', 'sen-travel' );
		...
		<p>
			...
			<?php
				wp_dropdown_categories(array(
					'show_option_none' => '',
					'class' 		  => 'widefat',
					'show_option_all'  => esc_html__('Popular Posts','sen-travel'),
	                                        ...
	                                        <div class="entry-container">
	                                            ...
	                                            <?php if ( 1 == $i ) : ?>
	                                                <div class="entry-content">
	                                                    <?php 
	                                                    	$excerpt = sen_travel_the_excerpt( 15 );
