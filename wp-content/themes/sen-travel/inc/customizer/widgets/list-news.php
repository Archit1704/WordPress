...
function sen_travel_sidebar_list_news() {
	register_widget( 'Sen_Travel_Sidebar_List_News' );
}
add_action( 'widgets_init', 'sen_travel_sidebar_list_news' );

class Sen_Travel_Sidebar_List_News extends WP_Widget{ 

	function __construct() {
		global $control_ops;
		$widget_sidebar_list = array(
		  'classname'   => 'sidebar-list-news',
		  'description' => esc_html__( 'Add Widget to Display List Posts.', 'sen-travel' )
		);
		parent::__construct( 'Sen_Travel_Sidebar_List_News',esc_html__( 'ST: List Posts', 'sen-travel' ), $widget_sidebar_list, $control_ops );
	}

	function form( $instance ) {
		$instance = wp_parse_args( (array) $instance, 
			array( 
			  'title'			=> esc_html__( 'List Posts', 'sen-travel' ),		
			  'category'       	=> '', 
			  'number'          => 5, 
			  'show_category'	=> true,	
			) 
		);
		$title     			= isset( $instance['title'] ) ? esc_html( $instance['title'] ) : esc_html__( 'List Posts', 'sen-travel' );
		...
		<p>
			...
			<?php
				wp_dropdown_categories(array(
					'show_option_none' => '',
					'class' 		  => 'widefat',
					'show_option_all'  => esc_html__('List Posts','sen-travel'),
